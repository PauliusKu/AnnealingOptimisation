...
double funcGoldsteinAndPrice(std::vector<double> &X) {
    double a, b;
    a = 1 +
        pow(X[0] + X[1] + 1, 2) * (19 - 14 * X[0] + 3 * pow(X[0], 2) - 14 * X[1] + 6 * X[0] * X[1] + 3 * pow(X[1], 2));
    b = 30 + pow(2 * X[0] - 3 * X[1], 2) *
...
double funcBeale(std::vector<double> &X) {
    return pow(1.5 - X[0] * (1 - X[1]), 2) + pow(2.25 - X[0] * (1 - pow(X[1], 2)), 2) +
           pow(2.625 - X[0] * (1 - pow(X[1], 3)), 2);
}

double funcBranin(std::vector<double> &X) {
    return pow(X[1] - (5.1 / (4 * pow(M_PI, 2))) * pow(X[0], 2) + 5 * X[0] / M_PI - 6, 2) +
           10 * (1 - 1 / (8 * M_PI)) * std::cos(X[0]) + 10;
}

double funcHump(std::vector<double> &X) {
    return 1.0316285 + 4 * pow(X[0], 2) - 2.1 * pow(X[0], 4) + pow(X[0], 6) / 3 + X[0] * X[1] - 4 * pow(X[1], 2) +
           4 * pow(X[1], 4);
}
